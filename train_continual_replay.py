...
def train_aux(logging_start_epoch, epoch, data, model, criterion, optimizer, ewc=None, prev_model=None, criterion_kd=None, data_aux=None):
    """Main training procedure.
    ...
    for i, (batch_cbs, batch_rrs) in enumerate(zip(data, data_aux)):
        ...
        # ### total loss from both rrs and cbs batch
        # loss = hp.cbs_importance * loss + hp.rrs_importance * loss_rrs
...
if __name__ == '__main__':
    ...
    for task_idx, train_lang in enumerate(hp.training_langs, 1):
        ...
        if hp.use_replay and (not hp.use_gem):
            ...
            elif hp.aux_imbalance_sampling:
