...
if __name__ == '__main__':
    ...
    if hp.continue_training and hp.initial_training:
        training_langs = hp.languages[:5]  # first 5 of the total 10 languages
    elif hp.continue_training:
        training_langs = hp.continue_languages   # the 1 langauge that is being trained
    ...
    if hp.multi_language and hp.balanced_sampling and hp.perfect_sampling:
        # merge training or initial training with multiple languages
        dp_devices = args.max_gpus if hp.parallelization and torch.cuda.device_count() > 1 else 1
        train_sampler = PerfectBatchSampler(dataset.train, training_langs, hp.batch_size, data_parallel_devices=dp_devices, shuffle=True, drop_last=True)
        train_data = DataLoader(dataset.train, batch_sampler=train_sampler, collate_fn=TextToSpeechCollate(hp.sort_by_text_len), num_workers=args.loader_workers) # was false
        eval_sampler = PerfectBatchSampler(dataset.dev, training_langs, hp.batch_size, data_parallel_devices=dp_devices, shuffle=False)
        eval_data = DataLoader(dataset.dev, batch_sampler=eval_sampler, collate_fn=TextToSpeechCollate(hp.sort_by_text_len), num_workers=args.loader_workers) # was false
    else:
        if hp.multi_language and hp.balanced_sampling:
            # merge training w/o perfect_sampling, i.e. non-grouped conv parameters as in
            # "simple", "separate", "shared"
            sampler = RandomImbalancedSampler(dataset.train)
        elif hp.multi_language and hp.balanced_sampling is False:
            # continue training case where only one language is under training
            # but the language one-hot embedding need to be correctly figured out
            # (i.e. globally unique language id)
            sampler = None
        else:
            # single language training without lang id
            sampler = None
    ...
    if args.checkpoint:
        ...
        print("model loaded from {}".format(os.path.join(args.base_directory, args.checkpoint_root, args.checkpoint)))
    ...
    if hp.use_ewc:
        ewc = EWC(model, criterion, train_data_old, hp.ewc_sample_size)
        state_dict = {
            'epoch': initial_epoch,
        ...
        torch.save(state_dict, "./shared_initial5_simple_onehot/checkpoints/SHARED-TRAINING_loss-299-0.119-fisher")
    else:
        ewc = None
